...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	 ...
	 * ApplicationContext 实例会创建并配置所有的单例 bean.
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 因为 beanDefinitionMap 是全局变量，这里定会存在并发访问的情况，所以用线程安全的map
		BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);
		// 处理 已经注册了 beanName 的情况
	...
	/**
	 * 寻找类型的匹配执行顺序时，首先尝试使用解析器进行解析，如果解析器没有成功解析，那么可能是使用默认的解析器没有做任何处理，
	 * 或者是使用了自定义的解析器，但是对于集合等类型来说并不在解析范围之内，所以再次对不同类型进行不同情况的处理
	 * @param descriptor
	 * @param beanName
	...
	private Object resolveMultipleBeans(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		if (type.isArray()) {
			...
			// 根据属性类型找到 beanFactory 中所有类型的匹配 bean, 返回值的构成为：key= 匹配的 beanName,
			// value=beanName 对应的实例化后的 bean(通过getBean(beanName) 返回)
