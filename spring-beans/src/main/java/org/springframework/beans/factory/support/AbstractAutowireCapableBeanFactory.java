...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 * 2.对 override 属性进行标记及验证(lookup-method 和 replace-method 对应的 BeanDefinition 中的 methodOverrides 属性)
	 ...
	 * 		2.1 如果存在工厂方法则使用工厂方法进行初始化
	 * 		2.2 一个类有多个构造函数，每个构造函数都有不同的参数，所以需要根据参数锁定构造函数并进行初始化
	 * 		2.3 如果既不存在工厂方法也不存在带有参数的构造函数，则使用默认的构造函数进行 bean 的实例化
	 * 3.MergedBeanDefinitionPostProcessor 的应用
	 * 		bean 合并后的处理， Autowired 注解正是通过此方法实现诸如类型的预解析
	 * 4.依赖处理
	 * 5.属性填充
	 * 6.循环依赖检查
	 * 		检测已经加载的 bean 是否已经出现了依赖循环，并判断是否需要抛出异常
	 * 7.注册 DisposableBean
	 * 		如果配置了 destroy-method, 这里需要注册以便于在销毁时候
	 * 8.完成创建并返回
	 *
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				...
				// 其中 Autowired 正是通过此方法实现诸如类型的预解析
				// (其中一个子类 AutowiredAnnotationBeanPostProcessor)
				try {
		...
		if (earlySingletonExposure) {
			...
			// (因为创建bean之后才加入一级缓存-在 , 而 allowEarlyReference 为 false, 所以此处是从二级缓存中获取的)
			if (earlySingletonReference != null) {
				...
				if (exposedObject == bean) {
					...
					// 所有依赖 beanName 的 bean 数组
					String[] dependentBeans = getDependentBeans(beanName);
					Set<String> actualDependentBeans = new LinkedHashSet<>(dependentBeans.length);
					for (String dependentBean : dependentBeans) {
						// 检测依赖 this.alreadyCreated.contains(beanName)
					 ...
					 *  actualDependentBeans 不为空则表示当前 bean 创建后其依赖的 bean 却没有全部创建完，也就是存在循环依赖
					 */
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		Object bean = null;
		// 如果尚未被解析 mbd.beforeInstantiationResolved 默认为null
		// Boolean.FALSE.equals(mbd.beforeInstantiationResolved) 判断结果为false
	 ...
	 * 实例化前的后处理器应用
	 *
	 * bean 的实例化前调用，也就是将 AbstractBeanDefinition 转换为 BeanWrapper 前的处理。
	 * 给子类一个修改 BeanDefinition 的机会，也就是说当程序经过这个方法后，
	 * bean 可能已经不是我们认为的 bean 了，而是或许成为了一个经过处理的代理的 bean,
	 ...
	 * 3.应用 InstantiationAwareBeanPostProcessor 处理器的 postProcessPropertyValues 方法，对属性获取完毕填充前对属性的再次处理，
	 * 	典型应用是 RequiredAnnotationBeanPostProcessor 类中对属性的验证
	 * 4.将所有 PropertyValues 中的属性填充
	 * @param beanName the name of the bean
	 * @param mbd the bean definition for the bean
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					// 返回值为是否继续填充 bean, 如果后处理器发出停止填充命令则终止后续的执行
	 ...
	 *
	 * 实现根据名称自动匹配和根据类型自动匹配的第一步都是寻找 bw 中需要依赖注入的属性，然后遍历这些属性并寻找类型匹配的 bean,
	 * 其中最复杂的就是寻找类型匹配的bean. 同时，Spring 中提供了对集合类型注入的支持，如使用注解的方式：
	 * @Autowired
	 * private List<Test> tests;
	 * Spring 会把所有与 Test 匹配的类型找出来并注入到 tests 属性中，所以在 autoByType 函数中，
	 * 新建了局部变量 autowiredBeanNames, 用于存储所有依赖的 bean, 如果只是对非集合类的属性注入来说，此属性并无用处
	 *
