...
public class BeanDefinitionParserDelegate {
	...
	public static final String REPLACED_METHOD_ELEMENT = "replace_method-method";
	...
	/**
	 * 在开始对属性进行全面解析前，Spring在外层又做了一个当前层的功能架构，在当前层完成的主要工作包含如下内容：
	 * 1.提取元素中的 id 以及 name 属性
	 * 2.进一步解析其他属性并统一封装到 GenericBeanDefinition 类型的实例中
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		// 2.进一步解析其他属性并统一封装到 GenericBeanDefinition 类型的实例中
		// GenericBeanDefinition 只是子类实现，而大部分的通用属性都存在了 AbstractBeanDefinition 中
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			// 它是把一个方法声明为返回某种类型的 bean，但实际要返回的 bean 是在配置文件里面配置的，
			// ********** 此方法可用在设计有些可插拔的功能上，解除程序依赖 **********
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			// ******解析 replace_method-method 属性*****
			// 方法替换：可以在运行时用新的方法替换现有的方法。与之前的 look-up 不同的是，
			// replace_method-method 不但可以动态地替换返回实体 bean, 而且还能动态地更改原有方法的逻辑。
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		// 如果有 singleton 属性，提示升级为 scope 属性
		...
		// 如果指定了 scope 属性
			...
			// 在嵌入 beanDefinition 情况下且没有单独指定 scope 属性，则使用父类默认的属性
	...
	/**
	 * Parse replace_method-method sub-elements of the given bean element.
	 */
	 ...
	 *
	 * 与构造函数注入方式不同的是将返回值使用 PropertyValue 进行封装，并记录在了 BeanDefinition 中的 propertyValues 属性中
	 */
	 ...
	 * 对于 qualifier 元素的获取，我们接触的更多的是注解的形式，在使用 Spring 框架中进行自动注入时，
	 * Spring 容器中匹配的候选 Bean 数目必须有且仅有一个。 找不到时，Spring 容器将抛出 BeanCreationException 异常，
	 * 并指出必须至少拥有一个匹配的 Bean.
	 *
	 * Spring 允许我们通过 Qualifier 指定注入 Bean 的名称，这样歧义就消除了，而对于配置方式使用：
	 * <bean id="myTestBean" class="bean.MyTestBean">
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// 解析 construct-arg 上的 value 属性
		boolean hasValueAttribute = ele.hasAttribute(VALUE_ATTRIBUTE);
		if ((hasRefAttribute && hasValueAttribute) ||
				((hasRefAttribute || hasValueAttribute) && subElement != null)) {
			/**
			 * 在 construct-arg 上不存在：
		...
		if (hasRefAttribute) {
			...
			// 既没有 ref 也没有 value 也没有子元素，Spring 蒙了
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(Element ele, BeanDefinitionHolder originalDef) {
		// 第三个参数是父类 bean, 当对某个嵌套配置进行分析时，这里需要传递父类 beanDefinition.
		// 这里传递的参数其实是为了使用父类的 scope 属性，以备子类若没有设置 scope 时默认使用父类的属性，
		// 这里分析的是顶层配置，所以为null
