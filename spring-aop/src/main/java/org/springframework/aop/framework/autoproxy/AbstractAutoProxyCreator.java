...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessAfterInitialization(@Nullable Object bean, String beanName) throws BeansException {
		if (bean != null) {
			// 根据给定的 bean 的 class 和 name 构建出一个 key，4.2.3 以前格式：beanClassName_beanName，
	 ...
	 * 对于代理类的创建及处理，委托给了 ProxyFactory 去处理，而在此函数中主要是对 ProxyFactory 的初始化操作，进而对真正的创建代理做准备，
	 * 这些初始化操作包括如下内容：
	 * 	1.获取当前类中的属性
	 * 	2.添加代理接口
	 ...
	 * 	6.进行获取代理操作
	 * 其中，封装 Advisor 并加入到 ProxyFactory 中以及创建代理是两个相对繁琐的过程，可以通过 ProxyFactory 提供的 addAdvisor 方法
	 * 直接将增强器置入代理创建工厂中，但是将拦截器封装为增强器还是需要一定的逻辑
	 *
	 * @param beanClass the class of the bean
	 * @param beanName the name of the bean
