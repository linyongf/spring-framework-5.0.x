 ...
 *
 * JDK 和 CGLIB 方式总结：
 * 1.如果目标对象实现了接口，默认情况下会采用 JDK 动态代理实现 AOP
 * 2.如果目标对象实现了接口，可以强制使用 CGLIB 实现 AOP
 * 3.如果目标对象没有实现接口，必须采用 CGLIB 库，Spring 会自动在 JDK 动态代理和 CGLIB之间转换
 *
 * 如何强制使用 CGLIB 实现 AOP：
...
public class DefaultAopProxyFactory implements AopProxyFactory, Serializable {
	 ...
	 * 		设置方式为<aop:aspectj-autoproxy proxy-target-class="true"/>或 @EnableAspectJAutoProxy(proxyTargetClass = true)
